...
impl NetworkController {
    pub fn new(
        ...
        // This ensures that the server is always at index 0
        result.network_graph.insert_node(node_id, NodeType::Server);
        result
    }
...
impl NetworkGraph {
    ...
    fn reset_graph(&self, server_node_id: NodeId) {
    ...
    pub fn delete_edge(&self, from: NodeId, to: NodeId) {
        let nodes = self.nodes.borrow();
    ...
    pub fn get_path_to(&self, to: NodeId) -> Vec<NodeId> {
        todo!()
    }
}
...
impl NetworkNode {
    ...
    fn insert_edge(&self, to: Rc<NetworkNode>) {
        self.neighbors.borrow_mut().push(to)
    }

    fn remove_edge(&self, to: Rc<NetworkNode>) {
    ...
    fn increment_dropped_packets(&mut self) {
